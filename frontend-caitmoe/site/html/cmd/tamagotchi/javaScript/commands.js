export async function processCommand(command) {
    ...
    if(command[0] == "play" || command[0] == "p") {
        let res = await getURL("https://cait.moe:8443/api/tamagotchi/play");
        clearOld();
        addToOld(command[0], await calculateTamagotchiString(JSON.parse(res).message));}
    else if(command[0] == "feed" || command[0] == "f") {
        let res = await getURL("https://cait.moe:8443/api/tamagotchi/feed");
        clearOld();
        addToOld(command[0], await calculateTamagotchiString(JSON.parse(res).message));}
    else if(command[0] == "rest" || command[0] == "s") {
        let res = await getURL("https://cait.moe:8443/api/tamagotchi/rest");
        clearOld();
        addToOld(command[0], await calculateTamagotchiString(JSON.parse(res).message));}
    //Utils
        ...
        await getURL("https://cait.moe:8443/api/tamagotchi/reload");
        clearOld();
    ...
    //Admin
        ...
        await getURL("https://cait.moe:8443/api/tamagotchi/new");
        clearOld();
...
async function getTamagotchi(res) {
    let data = JSON.parse(await getURL("https://cait.moe:8443/api/tamagotchi/tamagotchi"));
    let age;
    let info = "";
    if(data.state == "Dead") {
