...
async function updateWatchList(cameraId: number, state: boolean) {
  ...
  await postUpdate("https://cait.moe:8443/api/dgt/cameras/"+cameraId, apiPassword.value, state);
  await getCameras('https://cait.moe:8443/api/dgt/cameras/');
}
...
onMounted(() => {
  // Fetch camera list
  getCameras("https://cait.moe:8443/api/dgt/cameras/");
